...
class KGEModel(nn.Module):
    def __init__(self, args):
        ...
        self.w_e_s = nn.Parameter(torch.zeros(self.stt_dim, self.rel_dim // self.mp_dim))
        self.w_e_o = nn.Parameter(torch.zeros(self.stt_dim, self.rel_dim // self.mp_dim))
    ...
    def e_p_emb(self, e_emb, w_e):
        if self.mdl_nm in ['RotatE', 'ComplEx']:
            re_w_e, im_w_e = torch.chunk(w_e, 2, dim=0)
    ...
    def _transform(self, x, r, c, d_abs, m_abs, y_abs, w_e, w_rp):
        e = torch.index_select(self.e_emb, dim=0, index=x).unsqueeze(1)
        e_t = self.t_emb(x, d_abs, m_abs, y_abs).unsqueeze(1)
        e_p = self.e_p_emb(e, w_e)
        e_r = self.e_r_emb(r, c.view(-1, self.nr).contiguous(), w_rp)
        return e, e_t, e_p, e_r

    def _transform_neg(self, x, r, c, d_abs, m_abs, y_abs, w_e, w_rp):
        ...
        e_p = self.e_p_emb(
            e.view(x.size(0) * x.size(1), self.stt_dim).unsqueeze(1), w_e
    ...
    def _transform_tr_neg(self, x, r, c, t, d_abs_neg, m_abs_neg, y_abs_neg, w_rp):
        e_t_neg = self.t_emb(
    ...
    def forward(self, x, md=None):
        if md is None:
            ...
            s, s_t, s_p, s_r = self._transform(
                x[:, 0], x[:, 1], x[:, -self.nr * 2:-self.nr], d_abs, m_abs, y_abs, self.w_e_s, self.w_rp_s)
            r = torch.index_select(self.r_emb, dim=0, index=x[:, 1]).unsqueeze(1)
            o, o_t, o_p, o_r = self._transform(
                x[:, 2], x[:, 1], x[:, -self.nr:], d_abs, m_abs, y_abs, self.w_e_o, self.w_rp_o)
            t_neg = None
        elif md == 's':
            pos, neg, neg_abs, neg_abs_s_rel, neg_abs_o_rel, neg_rel = x

            d_abs, m_abs, y_abs = pos[:, 3].view(-1, 1), pos[:, 4].view(-1, 1), pos[:, 5].view(-1, 1)
            s, s_t, s_p, s_r = self._transform_neg(
                neg, pos[:, 1], neg_rel, d_abs, m_abs, y_abs, self.w_e_s, self.w_rp_s)
            r = torch.index_select(self.r_emb, dim=0, index=pos[:, 1]).unsqueeze(1)
            o, o_t, o_p, o_r = self._transform(
                pos[:, 2], pos[:, 1], pos[:, -self.nr:].contiguous(), d_abs, m_abs, y_abs, self.w_e_o, self.w_rp_o)
            ...
            s_p_neg = self.e_p_emb(true_s, self.w_e_s)
            ...
            s, s_t, s_p, s_r = self._transform(pos[:, 0], pos[:, 1], pos[:, -self.nr * 2: -self.nr].contiguous(),
                                               d_abs, m_abs, y_abs, self.w_e_s, self.w_rp_s)
            r = torch.index_select(self.r_emb, dim=0, index=pos[:, 1]).unsqueeze(1)
            o, o_t, o_p, o_r = self._transform_neg(neg, pos[:, 1], neg_rel,
                                                   d_abs, m_abs, y_abs, self.w_e_o, self.w_rp_o)
            ...
            o_t_neg, o_r_neg = self._transform_tr_neg(
                pos[:, 2], pos[:, 1], neg_abs_o_rel, neg_abs, d_abs_neg, m_abs_neg, y_abs_neg, self.w_rp_o)
            o_p_neg = self.e_p_emb(true_o, self.w_e_o)
    ...
    def _real(self, s, r, o, s_t, o_t, t_neg, md):
        s, o, s_neg, o_neg, r_neg = None, None, None, None, None
    ...
    def TransE(self, s, r, o, s_t, o_t, s_p, o_p, s_r, o_r, t_neg, md):
        s, o, s_neg, o_neg, r_neg = self._real(s, r, o, s_t, o_t, t_neg, md)

        if md == 's':
            sc_neg = s_neg + (r_neg - o_neg)
            sc = s + (r - o)

            sc = self._sc(sc, sc_neg)
        if md == 'o':
            sc_neg = (s_neg + r_neg) - o_neg
            sc = (s + r) - o
        ...
        sc = self.gamma - torch.norm(sc, p=1, dim=2)
    ...
    def RotatE(self, s, r, o, s_t, o_t, s_p, o_p, s_r, o_r, t_neg, md):
        ...
        sc = torch.stack([torch.cat([re_sc, re_b, re_c], dim=2), torch.cat(
            [im_sc, im_b, im_c], dim=2)], dim=0).norm(dim=0)
        sc = F.dropout(sc, p=self.drp, training=self.training)
