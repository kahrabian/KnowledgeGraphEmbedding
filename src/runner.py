...
def train_step(mdl, opt, opt_sc, tr_it, args):
    ...
    pos, neg, neg_abs, neg_rel, neg_abs_s_rel, neg_abs_o_rel, smpl_w, md = _cuda(args, next(tr_it))
...
def test_step(mdl, ts_q, al_q, ev_ix, tp_ix, tp_rix, e_ix, u_ix, args):
    ...
    with torch.no_grad():
        for ts_dl, md in ts_dls:
            for pos, neg, neg_abs, neg_abs_s_rel, neg_abs_o_rel, neg_rel, fil_b in map(lambda x: _cuda(args, x), ts_dl):
                if stp > tot_stp:
                    break
    ...
    return {mtr: sum([log[mtr] for log in logs]) / len(logs) for mtr in logs[0].keys()}
